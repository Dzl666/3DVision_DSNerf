...
def render(H, W, focal, chunk=1024*32, rays=None, c2w=None, ndc=True, near=0., far=1.,
            use_viewdirs=False, c2w_staticcam=None, depths=None, **kwargs):
    """Render rays
    Args:
      ...
      rays: array of shape [2, B, 3]. Ray origin and direction for each example in batch.
      c2w: array of shape [3, 4]. Camera-to-world transformation matrix.
      ndc: bool. If True, represent ray origin, direction in NDC coordinates.
      near: float or array of shape [B]. Nearest distance for a ray.
      far: float or array of shape [B]. Farthest distance for a ray.
      use_viewdirs: bool. If True, use viewing direction of a point in space in model.
      c2w_staticcam: array of shape [3, 4]. If not None, use this transformation matrix for 
       camera while using other c2w argument for viewing directions.
    Returns:
      rgb_map: [B, 3]. Predicted RGB values for rays.
      disp_map: [B]. Disparity map. Inverse of depth.
      acc_map: [B]. Accumulated opacity (alpha) along a ray.
      extras: dict with everything returned by render_rays().
    """
    if c2w is not None:
        rays_o, rays_d = get_rays(H, W, focal, c2w) # special case to render full image
    else:
...
def train(args):
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            if args.render_test_ray:
                ...
                rgb, sigma, z_vals, depth_maps, weights = render_test_ray(
                    rays_o, rays_d, hwf,
    ...
    if use_batching:
        ...
        raysRGB_iter = iter(DataLoader(
            RayDataset(rays_rgb), batch_size = N_rgb, shuffle=True, num_workers=0
            ))
    ...
    print(f"Begin - TRAIN views: {i_train}")
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            # Random over all images - batch = rays_rgb[i_batch:i_batch+N_rand] # [B, 2+1, 3*?]
            try:
        ...
        """
            ...
            if i%args.i_img==0:
                img_i=np.random.choice(i_val)# Log a rendered validation view to Tensorboard
                target = images[img_i]
                pose = poses[img_i, :3,:4]
        ...
        if i%args.i_testset==0 and i > 0 and len(i_test) > 0:
            ...
            with torch.no_grad():
                ...
                print("GPU memory used: {:3f} GB".format(torch.cuda.memory_allocated(device) / 1024**3))
                print("Max GPU memory used: {:3f} GB".format(torch.cuda.max_memory_allocated(device) / 1024**3))
...
def config_parser():
    ...
    parser.add_argument("--datadir", type=str, default='./data/8v_400_5_435', help='input data directory')
    ...
    parser.add_argument("--render_only", action='store_true',help='reload weights and render out render_poses path')
    parser.add_argument("--render_test", action='store_true', help='render the path of test set')
    parser.add_argument("--render_test_ray", action='store_true', help='render the path of test set')
    parser.add_argument("--render_train", action='store_true', help='render the path of train set')  
    parser.add_argument("--render_mypath", action='store_true', help='render the self-generated path')         
    parser.add_argument("--render_factor", type=int, default=0, 
                        help='downsampling factor to speed up rendering (4/8)')
    parser.add_argument("--render_itp_nodes", type=int, default=1, 
                        help='inserted nodes between two poses when interpolate')
    ...
    parser.add_argument("--dataset_type", type=str, default='llff', 
                        help='options: colmap_llff / llff / dtu')
    parser.add_argument("--testskip", type=int, default=8, 
                        help='load 1/N images from test/val sets, for large datasets')

    ## deepvoxels flags
    parser.add_argument("--shape", type=str, default='greek', help='options : armchair / cube / greek / vase')
    ## blender flags
    parser.add_argument("--white_bkgd", action='store_true', help='render synthetic data on a white bkgd (always use for dvoxels)')
    ...
    parser.add_argument("--seg_video", type=int, default=2, help="segmentation of generated video")
    # debug
    ...
    parser.add_argument("--test_scene", nargs='+', type=int, help='id of scenes used to test')
    parser.add_argument("--colmap_depth", action='store_true', help="Use depth supervision by colmap.")
    parser.add_argument("--depth_loss", action='store_true', help="Use depth supervision by colmap - depth loss.")
    parser.add_argument("--depth_lambda", type=float, default=0.1, help="Depth lambda used for loss.")
    parser.add_argument("--sigma_loss", action='store_true', help="Use depth supervision by colmap - sigma loss.")
    parser.add_argument("--sigma_lambda", type=float, default=0.1, help="Sigma lambda used for loss.")
    parser.add_argument("--weighted_loss", action='store_true', help="Use weighted loss by reprojection error.")
    parser.add_argument("--relative_loss", action='store_true', help="Use relative loss.")
    parser.add_argument("--depth_with_rgb", action='store_true', help="single forward for both depth and rgb")
    parser.add_argument("--normalize_depth", action='store_true', help="normalize depth before calculating loss")
    parser.add_argument("--depth_rays_prop", type=float, default=0.5, help="Proportion of depth rays.")
    return parser
...
if __name__=='__main__':
    ...
    print(f"======== Training using {device} now! ========")
    train(args)
